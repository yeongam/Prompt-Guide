#!/usr/bin/env python3
"""Sync compact GPT skill and hook cards from official OpenAI GitHub repositories.
...
HOOK_SOURCES: tuple[HookSource, ...] = (
    HookSource(
        ...
        checks=(
            "Resolve the latest official OpenAI source commit.",
    ...
    HookSource(
        ...
        checks=(
            ...
            "Preserve existing GPT skill behavior when adding agent guidance.",
        ),
        actions=(
    ...
    HookSource(
        repo="openai/codex",
        branch="main",
        purpose="Lightweight coding agent that runs in the terminal",
        hook_slug="codex-reference-application-guard",
        hook_name="Codex Reference Application Guard",
        event="session_reference",
        trigger="Use when Codex works from Prompt-Guide/GPT routine output.",
        checks=(
            "Treat the latest GPT skills and hooks snapshots as repo-local guidance.",
            "Do not assume GitHub Actions changed the live Codex runtime.",
...
def write_changelog(
    ...
    lines = [
        f"Prompt-Guide GPT Skills and Hooks Changelog - {today}",
        "",
        f"Snapshot: GPT/skills/{today}/skills",
        f"Hooks: GPT/hooks/{today}/hooks",
        ...
        *bullets(hook_diff["deleted"]),
        ...
        f"- 날짜별 훅 스냅샷 구조 유지: {hooks_dir.relative_to(GPT_ROOT)}",
