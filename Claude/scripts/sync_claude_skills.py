...
Mirrors the structure of GPT/scripts/sync_openai_skills.py so both product lines
follow the same dated-snapshot convention. Dependency-free and non-interactive
...
HOOK_SOURCES: tuple[HookSource, ...] = (
    ...
    HookSource(
        ...
        checks=(
            "Confirm tools, permissions, and orchestration guidance stay separated.",
    ...
    HookSource(
        ...
        branch="main",
        purpose="Anthropic's official CLI for Claude Code",
        hook_slug="claude-code-reference-application-guard",
        hook_name="Claude Code Reference Application Guard",
...
def write_changelog(
    ...
    lines = [
        ...
        f"CLI changelog version: {catalog_version}",
        "",
        "[추가된 스킬]",
        ...
        *bullets(hook_diff["deleted"]),
        ...
        "- SKILLS_CATALOG.yaml은 CLI 버전/명령어 요약용으로 유지, 상세본은 날짜 스냅샷에 위임",
        "",
        "[토큰 절감 관련 변경 사항]",
...
def main() -> int:
    ...
    catalog_version = version_file.read_text().strip() if version_file.exists() else "unknown"
    write_changelog(today, skill_diff, hook_diff, skills_dir, hooks_dir, catalog_version)
