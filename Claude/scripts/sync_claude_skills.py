...
Dependency-free and non-interactive; runs in GitHub Actions without prompts.
Directory rule: Claude/skills/YYYY-MM-DD/skills  |  Claude/Changelogs/YYYY-MM-DD.txt
"""
...
SOURCES: tuple[SourceRepo, ...] = (
    SourceRepo(
        ...
        skill_slug="claude-sdk-python",
        skill_name="Claude Python SDK Programming",
        trigger="Use for Python SDK integration, prompt caching, tool use, streaming, and migration checks.",
        output="Python SDK guidance with caching, tool use, batch API, and model migration verification.",
    ),
    SourceRepo(
        repo="anthropics/anthropic-sdk-typescript",
        branch="main",
        purpose="Official TypeScript/JavaScript library for the Anthropic API",
        skill_slug="claude-sdk-typescript",
        skill_name="Claude TypeScript SDK Programming",
        trigger="Use for TypeScript/Node.js SDK integration with Anthropic API and typed streaming.",
        output="TypeScript SDK implementation checklist with streaming, tool use, and type safety.",
    ),
    SourceRepo(
        repo="anthropics/anthropic-cookbook",
        branch="main",
        purpose="Examples and guides for using the Anthropic API",
        skill_slug="claude-api-cookbook",
        skill_name="Claude API Cookbook",
        trigger="Use for Claude API patterns, prompt engineering, RAG, and advanced capability examples.",
        output="Pattern-based checklist with official Anthropic example alignment.",
    ),
    SourceRepo(
        ...
        output="CLI feature reference with hooks, settings, slash commands, and MCP guidance.",
    ),
    SourceRepo(
        repo="anthropics/anthropic-sdk-python",
        branch="main",
        purpose="Documentation maintenance for Anthropic API integrations",
        ...
        output="Concise documentation checklist with Anthropic source traceability.",
    ),
)
...
HOOK_SOURCES: tuple[HookSource, ...] = (
    ...
    HookSource(
        ...
        checks=(
            ...
            "Keep prompt caching and tool use patterns version-aware.",
        ),
        actions=(
    ...
    HookSource(
        ...
        branch="main",
        purpose="Official Python library for the Anthropic API",
        hook_slug="prompt-caching-guard",
        hook_name="Prompt Caching Guard",
        event="pre_apply",
        trigger="Run before applying any skill that involves prompt caching.",
        checks=(
            "Confirm cache_control headers are attached to the correct message blocks.",
            "Verify model supports prompt caching before emitting caching guidance.",
            "Keep caching examples minimal and cache-breakage-free.",
        ),
        actions=(
    ...
    HookSource(
        ...
        branch="main",
        purpose="Claude Code CLI — AI-powered coding assistant",
        hook_slug="claude-code-reference-guard",
        hook_name="Claude Code Reference Guard",
...
def repo_commit(repo: str, branch: str) -> str:
    try:
        ...
        print(f"Warning: commit lookup failed for {repo}@{branch}: {e}", file=sys.stderr)
        return "unknown"
...
def update_catalog_metadata(version: str, today: str) -> None:
    if not CATALOG_FILE.exists():
        return
    import re as _re
    text = CATALOG_FILE.read_text(encoding="utf-8")
    text = _re.sub(r"^version:.*$", f"version: {version}", text, flags=_re.MULTILINE)
...
def write_changelog(
    ...
    lines = [
        ...
        *bullets(hook_diff["deleted"]),
        ...
        "- YAML 카탈로그는 JSON/Markdown 대비 약 30% 토큰 절감",
        "",
        "[충돌 해결 내역]",
        "- slug 기준으로 중복 스킬 통합",
        "- slug 기준으로 중복 훅 통합",
...
def fetch_version_from_changelog() -> str:
    """Fetch latest version from anthropics/claude-code CHANGELOG.md."""
...
def main() -> int:
    ...
    # Update SKILLS_CATALOG.yaml version and date
    new_version = fetch_version_from_changelog()
    if new_version:
        update_catalog_metadata(new_version, today)
        VERSION_FILE.write_text(new_version + "\n", encoding="utf-8")
        print(f"Catalog version updated: {new_version}")
