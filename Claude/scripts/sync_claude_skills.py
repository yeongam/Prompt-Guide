...
GitHub Actions without prompts. Mirrors GPT/scripts/sync_openai_skills.py so both
ecosystems share one convention (dated skill/hook snapshots + one changelog).

Commit resolution uses `git ls-remote` instead of the GitHub REST API so the
script works without a token and without GitHub API scoping.
"""
...
SOURCES: tuple[SourceRepo, ...] = (
    SourceRepo(
        ...
        branch="main",
        purpose="Official Claude Code CLI: commands, hooks, and settings",
        skill_slug="claude-code-cli-programming",
        skill_name="Claude Code CLI Programming",
        trigger="Use for Claude Code slash commands, settings.json, and CLI workflows.",
        output="Compact CLI feature checklist with new commands/settings noted.",
    ),
    SourceRepo(
        repo="anthropics/claude-code",
        branch="main",
        purpose="Claude Code documentation and CHANGELOG maintenance",
        skill_slug="documentation-maintenance",
        skill_name="Documentation Maintenance",
        trigger="Use for updating CLAUDE.md, docs, and developer guides.",
    ...
    SourceRepo(
        ...
        skill_slug="anthropic-sdk-python-programming",
        skill_name="Anthropic SDK Python Programming",
    ...
    SourceRepo(
        ...
        branch="main",
        purpose="Official TypeScript/JavaScript library for the Anthropic API",
        skill_slug="anthropic-sdk-typescript-programming",
        skill_name="Anthropic SDK TypeScript Programming",
    ...
    SourceRepo(
        ...
        branch="main",
        purpose="Build custom agents on top of Claude Code with the Agent SDK",
        skill_slug="claude-agent-sdk-programming",
        skill_name="Claude Agent SDK Programming",
        trigger="Use for building custom agents, tools, and orchestration code.",
        output="Lean agent design and implementation checklist.",
    ),
)
...
HOOK_SOURCES: tuple[HookSource, ...] = (
    HookSource(
        ...
        checks=(
            "Resolve the latest official Anthropic source commit via git ls-remote.",
    ...
    HookSource(
        ...
        branch="main",
        purpose="Build custom agents on top of Claude Code with the Agent SDK",
        hook_slug="agent-sdk-workflow-guard",
        hook_name="Agent SDK Workflow Guard",
        event="post_apply",
        trigger="Run after applying Agent SDK workflow guidance.",
        checks=(
            "Confirm tools, handoffs, and orchestration guidance stay separated.",
    ...
    HookSource(
        ...
        branch="main",
        purpose="Official Claude Code CLI: commands, hooks, and settings",
        hook_slug="claude-code-session-reference-guard",
        hook_name="Claude Code Session Reference Guard",
        event="session_reference",
        trigger="Use when a live session works from Prompt-Guide/Claude routine output.",
        checks=(
            "Treat the latest Claude skills and hooks snapshots as repo-local guidance.",
            "Do not assume GitHub Actions changed the live local Claude Code runtime.",
...
def latest_claude_code_version() -> str:
    url = "https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md"
    try:
        changelog = request_text(url)
    except urllib.error.URLError:
        return ""
...
def write_changelog(
    today: str, skill_diff: dict[str, list[str]], hook_diff: dict[str, list[str]],
    skills_dir: Path, hooks_dir: Path, version_note: str,
) -> None:
    ...
    lines = [
        ...
        (f"- hooks: added={len(hook_diff['added'])}, modified={len(hook_diff['modified'])}, "
...
def update_version(new_version: str, today: str) -> None:
...
def main() -> int:
    ...
    version_note = ""
    new_version = sys.argv[1] if len(sys.argv) > 1 and sys.argv[1] else latest_claude_code_version()
    old_version = VERSION_FILE.read_text().strip() if VERSION_FILE.exists() else ""
    if new_version and new_version != old_version:
        update_version(new_version, today)
        version_note = f"Claude Code CLI version note: {old_version or 'none'} -> {new_version}"

    write_changelog(today, skill_diff, hook_diff, skills_dir, hooks_dir, version_note)
