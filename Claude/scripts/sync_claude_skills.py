...
Mirrors the layout and conventions of GPT/scripts/sync_openai_skills.py so the
Claude and GPT sides of this repo stay structurally consistent. Dependency-free
and non-interactive so it can run unattended in a daily routine.
"""
...
SOURCES: tuple[SourceRepo, ...] = (
    SourceRepo(
        ...
        branch="main",
        purpose="Official Claude Code CLI agent",
        skill_slug="claude-code-cli-coding",
        skill_name="Claude Code CLI Coding",
        trigger="Use for implementing or debugging work done through the Claude Code CLI itself.",
        output="Small code-oriented checklist aligned with the official CLI's current behavior.",
    ),
    SourceRepo(
...
def write_changelog(today: str, skill_diff: dict[str, list[str]], skills_dir: Path) -> None:
    ...
    lines = [
        ...
        *bullets(skill_diff["deleted"]),
        ...
        "- 기존 SKILLS_CATALOG.yaml(단일 파일 인덱스)은 유지하여 기존 기능과의 호환성 보존",
        "",
        "[토큰 절감 관련 변경 사항]",
...
def main() -> int:
    ...
    for source in SOURCES:
        ...
        skills.append(build_skill(source, readme))
