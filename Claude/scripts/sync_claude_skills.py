...
SOURCES: tuple[SourceRepo, ...] = (
    SourceRepo(
        ...
        branch="main",
        purpose="Official Claude Code agentic coding CLI",
        ...
        output="Compact CLI usage and skill/hook authoring checklist.",
    ),
    SourceRepo(
        ...
        trigger="Use for Python Anthropic SDK integration, request structure, and migration checks.",
    ...
    SourceRepo(
        ...
        trigger="Use for Node.js or TypeScript Anthropic SDK integration and typed API work.",
    ...
    SourceRepo(
        ...
        trigger="Use for Claude Agent SDK workflows, tools, and orchestration code.",
...
def write_changelog(today: str, skill_diff: dict[str, list[str]], skills_dir: Path) -> None:
    ...
    lines = [
        ...
        *bullets(skill_diff["deleted"]),
        ...
        "- 기존 SKILLS_CATALOG.yaml(슬래시 커맨드 카탈로그)과 슬러그 네임스페이스 분리로 충돌 없음",
        "- 변경 감지는 hash 비교로 수행",
        "",
        "[요약]",
        (
...
def main() -> int:
    ...
    for source in SOURCES:
        commit = source_commit(source.repo, source.branch)
