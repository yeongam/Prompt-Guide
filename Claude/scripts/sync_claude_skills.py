...
Mirrors GPT/scripts/sync_openai_skills.py so both directories share one maintainable
pattern. Dependency-free and non-interactive so it can run in GitHub Actions.
"""
...
SOURCES: tuple[SourceRepo, ...] = (
    SourceRepo(
        ...
        branch="main",
        purpose="Agentic coding tool that lives in the terminal and understands your codebase",
        skill_slug="claude-code-cli-programming",
        skill_name="Claude Code CLI Programming",
        trigger="Use for Claude Code CLI workflows, slash commands, and terminal-based coding tasks.",
        output="Minimal CLI workflow guidance with official-command alignment.",
    ),
    SourceRepo(
        repo="anthropics/claude-agent-sdk-python",
        branch="main",
        purpose="Python SDK for building agents on top of Claude Code",
    ...
    SourceRepo(
        ...
        branch="main",
        purpose="TypeScript/Node SDK for building agents on top of Claude Code",
        skill_slug="typescript-agent-sdk-programming",
        skill_name="TypeScript Agent SDK Programming",
        trigger="Use for Node.js or TypeScript Claude Agent SDK integration.",
        output="Compact TypeScript Agent SDK implementation checklist.",
    ),
    SourceRepo(
        repo="anthropics/anthropic-sdk-python",
        branch="main",
        purpose="Official Python client for the Claude (Messages) API",
        skill_slug="anthropic-api-coding",
        skill_name="Anthropic API Coding",
        trigger="Use for implementing or debugging direct Claude API calls.",
        output="Small code-oriented checklist with official-example alignment.",
    ),
    SourceRepo(
        repo="anthropics/claude-cookbooks",
        branch="main",
        purpose="Guides and copy-able code examples for building with Claude",
...
HOOK_SOURCES: tuple[HookSource, ...] = (
    ...
    HookSource(
        ...
        checks=(
            "Verify the Python Agent SDK source commit used by the routine.",
    ...
    HookSource(
        ...
        checks=(
            "Verify the TypeScript Agent SDK source commit used by the routine.",
    ...
    HookSource(
        ...
        branch="main",
        purpose="Official Python client for the Claude (Messages) API",
        hook_slug="api-coding-guard",
        hook_name="API Coding Guard",
        event="post_apply",
        trigger="Run after applying direct Claude API guidance.",
        checks=(
            "Confirm request/response shapes match the current SDK release.",
            "Prefer minimal runnable examples over speculative usage.",
            "Preserve existing Claude skill behavior when adding API guidance.",
        ),
        actions=(
    ...
    HookSource(
        ...
        branch="main",
        purpose="Agentic coding tool that lives in the terminal and understands your codebase",
        hook_slug="claude-code-runtime-guard",
        hook_name="Claude Code Runtime Guard",
...
def repo_commit(repo: str, branch: str) -> str:
    """Resolve the latest commit sha via `git ls-remote` (no API token required)."""
    try:
        out = subprocess.run(
...
def write_changelog(
    ...
    if CHANGELOGS_ROOT.exists() and not CHANGELOGS_ROOT.is_dir():
        raise RuntimeError(f"{CHANGELOGS_ROOT} exists and is not a directory")
